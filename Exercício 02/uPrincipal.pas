...
begin
  try
    ...
    if LocString = '' then //Sai se não estiver frase
      Exit;
...
begin
  ...
  if LocFrase = '' then
    ShowMessage('A sub frase "'+prmfrase+'" não foi encontrada em nenhuma das frases aramazenadas')
  else
...
begin
  ...
  begin
    ShowMessage('Não existe frases armazenadas, favor inserir');
    btnInserir.SetFocus();
    Exit();
  end;
